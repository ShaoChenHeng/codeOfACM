...
int n, r, c, d[maxd][maxd], d2[maxd][maxd], ans[maxd][maxd], cols[maxd];

void copy( char type, int p, int q )
{
    if ( type == 'R' ) for ( int i = 1; i <= c; i ++ ) d[p][i] = d2[q][i];
    else for ( int i = 1; i <= r; i ++ ) d[i][p] = d2[i][q];
}
...
int main()
{
    ...
    while ( scanf("%d %d %d", &r, &c, &n) == 3 && r )
    {
        ...
        for ( int i = 1; i <= r; i ++ ) 
        {
            for ( int j = 1; j <= c; j ++ ) d[i][j] = i * BIG + j;
        }
        while ( n -- )
        {
            scanf("%s",cmd);
